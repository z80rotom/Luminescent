...
Result fsdevMountSaveData(const char *name, uint64_t application_id, AccountUid uid);

/// Mounts the specified SaveData as ReadOnly.
/// Only available on [2.0.0+].
Result fsdevMountSaveDataReadOnly(const char *name, uint64_t application_id, AccountUid uid);

/// Mounts the specified BcatSaveData.
Result fsdevMountBcatSaveData(const char *name, uint64_t application_id);

/// Mounts the specified DeviceSaveData.
Result fsdevMountDeviceSaveData(const char *name, uint64_t application_id);
...
/// Only available on [3.0.0+].
Result fsdevMountCacheStorage(const char *name, uint64_t application_id, uint16_t save_data_index);

/// Mounts the specified SystemSaveData.
Result fsdevMountSystemSaveData(const char *name, FsSaveDataSpaceId save_data_space_id, uint64_t system_save_data_id, AccountUid uid);

/// Mounts the specified SystemBcatSaveData.
/// Only available on [4.0.0+].
Result fsdevMountSystemBcatSaveData(const char *name, uint64_t system_save_data_id);
...
Result fsdevCreateFile(const char* path, size_t size, uint32_t flags);
