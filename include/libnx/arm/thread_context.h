...
typedef struct {
    ...
    uint64_t fp;                     ///< Frame pointer (x29) (AArch64). For AArch32, check r11.
    uint64_t lr;                     ///< Link register (x30) (AArch64). For AArch32, check r14.
    uint64_t sp;                     ///< Stack pointer (AArch64). For AArch32, check r13.
    CpuRegister pc;             ///< Program counter.
    uint32_t         psr;            ///< PSTATE or cpsr.

    FpuRegister fpu_gprs[32];   ///< 32 general-purpose NEON registers.
    uint32_t         fpcr;           ///< Floating-point control register.
    uint32_t         fpsr;           ///< Floating-point status register.

    uint64_t         tpidr;          ///< EL0 Read/Write Software Thread ID Register.
} ThreadContext;

/// Thread exception dump structure.
typedef struct {
    uint32_t error_desc;             ///< See \ref ThreadExceptionDesc.
    uint32_t pad[3];
...
typedef struct {
    uint32_t cpu_gprs[8]; ///< GPRs 0..7.
    uint32_t sp;
    uint32_t lr;
    uint32_t elr_el1;
    uint32_t tpidr_el0;    ///< tpidr_el0 = 1
    uint32_t cpsr;         ///< cpsr & 0xFF0FFE20
    uint32_t afsr0;
    uint32_t afsr1;
    uint32_t esr;
    uint32_t far;
